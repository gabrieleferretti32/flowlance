...
| **L'IVA è una seconda componente, e usa un metodo leggermente diverso.** Le imposte distribuiscono un **residuo** — `fabbisognoDaAccantonare`, già al netto dei versamenti — mentre l'IVA prende ogni scadenza futura **al suo valore pieno**, perché `calcolaIva` liquida i periodi e non sa niente di quello che è stato versato. La conseguenza: **un trimestre già scaduto e non versato non compare nella quota.** Le scadenze passate si saltano, e nessuno può dire se sono state pagate. Per le imposte lo stesso caso è coperto — il residuo lo contiene e finisce nella voce «scadenza già passata» — per l'IVA no. Si chiuderà quando la quota potrà leggere i versamenti di tipo IVA | sì per quello che c'è, `accantonamento.test.ts`: componenti separate, totale uguale alla somma, IVA zero in forfettario. Il trimestre scaduto e non versato **no**, ed è questa riga |
